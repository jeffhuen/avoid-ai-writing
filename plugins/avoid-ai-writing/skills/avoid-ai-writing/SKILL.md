...
You are editing prose to remove generic or mechanical habits while preserving its information, voice, genre, and project rules.

## What this skill is and isn't

This is a **writing-quality tool**, not an authorship verdict. Humans writing under deadline pressure, in unfamiliar genres, or in a second language produce many of the same shapes. Independent audits of commercial AI detectors have found false-positive rates above 60% on non-native English writers (Liang et al., Stanford, *Patterns* 2023) and overall misclassification rates above 70% on open-source detectors (Jabarian & Imas, BFI Working Paper 2025-116, 2025). Adversarial paraphrase reduces detection accuracy by about 88% across every method tested (arXiv:2506.07001, 2025).
...
**Invocation.** Natural language is enough ("rewrite this in a blunt voice for LinkedIn," "edit `post.md` in place," "scan this, don't rewrite"). Power users can also pass explicit options, which map to the sections below: `[--mode rewrite|detect|edit]`, `[--voice casual|professional|technical|warm|blunt]`, `[--context linkedin|blog|technical-blog|investor-email|docs|casual]`, `[--source plain|rendered-markdown]`, `[--file PATH]`, `[--iterate N]` (max 2), `[--style CONFIG|GUIDE]`.

**Source view.** Use `--source rendered-markdown` when the input is a Markdown source file and the audit should judge only what a reader sees. Ignore YAML frontmatter and HTML comments in the audit and rewrite. A project profile may also protect headings, every blockquote, inline quotations, tables, and code. The masks preserve later character offsets. `plain` remains the default for pasted prose and backward compatibility.

## Project editorial profiles

A project profile outranks the generic catalog. Read it before auditing or rewriting, then apply its layers in this order:

1. **Protected material.** Exclude source-only comments and metadata, quotations, headings, tables, code, or other material the profile marks as reader-owned or non-prose.
2. **Hard rules.** These are exact project laws. They may fail a check.
3. **Positive craft.** Preserve the project's scene work, argument shape, pacing, humor, tension, terminology, and authorial distance. Negative rules cannot define a voice.
4. **Project advisories.** Review these with judgment. They do not fail a check.
5. **Generic patterns.** Treat the catalog below as unscored suggestions. A generic finding never overrides a project rule or forces an edit.

For long-form work, use the space the reader's experience requires. Do not optimize prose for token count or remove a sentence that carries scene, character, tension, orientation, rhythm, humor, emotional pressure, transition, or motif. Compression advice written for posts and emails does not govern narrative prose.

The honesty layer travels across genres. Never invent biography, feelings, scenes, quotations, expertise, motives, numbers, or authority. Replace puffery with the fact that earns the claim. If the record does not establish a detail, name the limit or leave a verification marker according to the project's rules.
...
Words are organized into three tiers based on editorial value and inherited frequency claims. The tiers help an editor distinguish a strong generic-language prompt from ordinary words that matter only when they cluster. They do not establish authorship. The structure is adapted from [brandonwise/humanizer](https://github.com/brandonwise/humanizer).
...
Multi-word boilerplate is often unobjectionable once and numbing in clusters. Crypto, web3, DePIN, and AI infrastructure reviews are common habitats. Flag **2+ uses of the same phrase**, plus a cluster of three or more distinct phrases from this table in one piece. The editorial problem is repeated abstraction, regardless of who produced it.
...
- Also watch for: "In this article, we will explore…" or "Let's dive in!" These are generic meta-narration. Cut or rewrite with a direct opening.
...
- Bracketed slot-fillers that were meant to be replaced before publishing: `[Your Name]`, `[INSERT SOURCE URL]`, `[Describe the specific section]`, `2025-XX-XX`, `<!-- Add citation if available -->`. Treat any visible placeholder as a publishing bug: fill it in with real content or delete the sentence entirely.
...
These aren't individual word or phrase problems. They concern how the text flows as a whole. Metronomic prose tires a reader, while deliberate variation carries emphasis and pace.

**Structure matters more than word substitution.** Replacing vocabulary cannot repair uniform pacing, repeated sentence construction, or symmetrical phrasing. Read the passage for movement and cadence after the word-level edit.
...
- **Vocabulary repetition vs. synonym cycling**: Repeat the right word when precision matters. Vary it when the meaning changes, not to avoid repetition by itself.
- **Read-aloud test**: Read for a cadence that fits the genre. Rewrite the stretches whose rhythm makes every sentence feel interchangeable.
- **Missing authorial stance**: Preserve opinions, preferences, and reactions when the writer supplied them and the genre permits them. Deliberately impersonal work does not need first person, and the editor must never invent it.
- **Over-polishing**: Aggressively editing out every irregularity can erase the writer's cadence. Preserve intentional roughness, idiosyncratic word choices, and uneven pacing when they carry the voice. Applying every generic rule at maximum strictness creates the uniformity this section warns against.

### Vocabulary diversity (stylometric)

In longer pieces (200+ words), look at how much vocabulary the text uses. The type-token ratio (TTR), distinct word types divided by total tokens, is a classical stylometric measure. The bundled analyzer conservatively flags samples under 0.40 for review.

A very low TTR is not an authorship finding. Narrow topics, technical reference material, and second-language writing all compress vocabulary. On general prose where range would help, a TTR below 0.40 can prompt a second look. The fix is rarely a thesaurus. Broaden the *what*: name specific things, cite specific cases, or replace a reused abstract noun with the concrete instance behind it.
...
- A writer-side diagnostic, not a regex: can you swap two body paragraphs without breaking the piece? If the order doesn't matter, you may have written a list of points instead of an argument that builds. The test targets self-contained modules with no necessary connection to their neighbors.
...
- Another writer-side test: read each paragraph and ask "what's actually new here?" Generic prose restates the premise in fresh words instead of advancing it. The tell is that a large cut loses no information or reader experience.
...
If the text has 5+ vocabulary hits across multiple categories, 3+ distinct pattern categories, and uniform sentence or paragraph length, patching phrases may not fix it. The structure itself is generic. Advise a full rewrite: state the core point in one sentence, then rebuild from there.
...
3. **Preserve the voice** — keep first person, preferences, and reactions only when the source or project supplies them.
4. **Keep the position** — if the source takes a position, do not neutralize it.
5. **Earn your emphasis** — don't tell the reader something is interesting. Make it interesting.

Removal is half the job. A rewrite that clears every flag but reads sterile has failed. Preserve the reactions, preferences, asides, and unresolved thoughts the writer supplied. Do not manufacture them. For encyclopedic, technical, legal, or deliberately impersonal work, neutral and plain may be the intended voice. Adapted from `blader/humanizer` ("Personality and soul").
